...
class BookingRepository extends BaseRepository
{
        ...
        if (count($jobs) > 0) {
            ...
            foreach ($jobs as $job) {
                if ($job->immediate === 'yes') {
                    array_push($emergencyJobs, $job);
                } else {
                    array_push($normalJobs, $job);
                }
            }

            $normalJobs = collect($normalJobs)->each(function ($item, $key) use ($userId) {
                $item['user_check'] = Job::checkParticularJob($userId, $item);
            })->sortBy('due')->all();
        }

        return ['emergency_jobs' => $emergencyJobs, 'normal_jobs' => $normalJobs, 'customer_user' => $customerUser, 'user_type' => $userType];
    }
    ...
    public function getUsersJobsHistory($userId, Request $request)
    {
        ...
        $consumerUser = User::find($userId);
        $emergencyJobs = $normalJobs = [];

        if(!empty($consumerUser)){
            if ($consumerUser->is('customer')) {
                $jobs = $consumerUser->jobs()->with('user.userMeta', 'user.average', 'translatorJobRel.user.average', 'language', 'feedback', 'distance')->whereIn('status', ['completed', 'withdrawbefore24', 'withdrawafter24', 'timedout'])->orderBy('due', 'desc')->paginate(15);
                return ['emergency_jobs' => $emergencyJobs, 'normal_jobs' => [], 'jobs' => $jobs, 'consumer_user' => $consumerUser, 'user_type' => 'customer', 'num_pages' => 0, 'page_num' => 0];
            } elseif ($consumerUser->is('translator')) {
                $jobsIds = Job::getTranslatorJobsHistoric($consumerUser->id, 'historic', $pageNum);
                $totalJobs = $jobsIds->total();
                $numPages = ceil($totalJobs / 15);

                return ['emergency_jobs' => $emergencyJobs, 'normal_jobs' => $jobsIds, 'jobs' => $jobsIds, 'consumer_user' => $consumerUser, 'user_type' => 'translator', 'num_pages' => $numPages, 'page_num' => $pageNum];
            }
        }
        return  [];
    }
    ...
    public function store($user, $data)
    {
        ...
        if ($user->user_type === env('CUSTOMER_ROLE_ID')) {
            ...
            $job = $consumerUser->jobs()->create($data);
            ...
            $data['customer_town'] = $consumerUser->userMeta->city;
            $data['customer_type'] = $consumerUser->userMeta->customer_type;
    ...
    public function jobEnd($postData = array())
    {
        ...
        $job_detail = Job::with('translatorJobRel')->find($postData["job_id"]);
    ...
    public function getPotentialJobIdsWithUserId($user_id)
    {
        ...
        $jobIds = Job::getJobs($user_id, $jobType, 'pending', $userLanguage, $gender, $translator_level);
    ...
    {
        $users = User::all();
        $translatorArray = $delPayTranslatorArray = array();            // suitable translators (no need to delay push) || suitable translators (need to delay push)
     ...
     * @return mixed
     */
    public function updateJob($id, $data, $consumerUser)
    {
        $job = Job::find($id);
        ...
        $this->logger->addInfo('USER #' . $consumerUser->id . '(' . $consumerUser->name . ')' . ' has been updated booking <a class="openjob" href="/admin/jobs/' . $id . '">#' . $id . '</a> with data:  ', $logData);
        ...
        if ($job->due <= Carbon::now()) {
            ...
            if ($changeTranslator['translatorChanged']) $this->sendChangedTranslatorNotification($job, $changeTranslator, $changeTranslator['new_translator']);
    ...
    private function changeTimedoutStatus($job, $data, $changedTranslator)
    {
        ...
        if ($data['status'] === 'pending') {
            ...
            $this->sendNotificationTranslator($job, $this->jobToData($job), '*');   // send Push all sutiable translators
    ...
    private function changeTranslator($currentTranslator, $data, $job)
    {
        ...
        if (!is_null($currentTranslator) || (isset($data['translator']) && !empty($data['translator'])) || !empty($data['translator_email'])) {
            $log_data = [];
            if (!is_null($currentTranslator) && ((isset($data['translator']) && $currentTranslator->user_id !== $data['translator']) || !empty($data['translator_email'])) && (isset($data['translator']) && !empty($data['translator']) )) {
                ...
                $log_data[] = [
                    ...
                    'new_translator' => $currentTranslator->user->email
                ];
                $translatorChanged = true;
            } elseif (is_null($currentTranslator) && isset($data['translator']) && (!empty($data['translator']) || !empty($data['translator_email']))) {
                if (!empty($data['translator_email'])) $data['translator'] = User::where('email', $data['translator_email'])->first()->id;
    ...
    {
        if ($oldDue !== $newDue) {
            return ['dateChanged' => true, 'log_data' => ['old_due' => $oldDue, 'new_due' => $newDue]];
        }
...
//        $jobs = TeHelper::convertJobIdsInObjs($job_ids);
    ...
    public function getAll($authUser, $requestData, $limit = null)
    {

        if ($authUser && $authUser->user_type === env('SUPER_ADMIN_ROLE_ID')) {
            $allJobs = Job::query();
            ...
            if (isset($requestData['filter_time_type']) && $requestData['filter_time_type'] === "created") {
            ...
            if (isset($requestData['filter_time_type']) && $requestData['filter_time_type'] === "due") {
                if (isset($requestData['from']) && $requestData['from'] !== "") {
